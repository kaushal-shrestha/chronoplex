...
private struct ChronoplexSnapshot: Codable {
    var nextId: Int64 = 1
...
final class ChronoplexStore: ObservableObject {
    ...
    private let key = "chronoplex.snapshot.v1"
    ...
    private func load() {
        guard let data = UserDefaults.standard.data(forKey: key),
              let snapshot = try? JSONDecoder().decode(ChronoplexSnapshot.self, from: data) else {
            return
        }
    ...
    private func persist() {
        ...
        let snapshot = ChronoplexSnapshot(
            nextId: nextId,
            clocks: clocks,
            alarms: alarms,
            timers: timers,
