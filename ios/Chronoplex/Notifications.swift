...
enum NotificationScheduler {
    private static let alarmCategory = "chronoplex.alarm"
    private static let timerCategory = "chronoplex.timer"
    ...
    private static func scheduleAlarmRequest(alarm: AlarmEntry, fireMillis: Int64, suffix: String) {
        let content = UNMutableNotificationContent()
        content.title = alarm.label.trimmed.isEmpty ? "Chronoplex alarm" : alarm.label
...
final class NotificationRouter: NSObject, UNUserNotificationCenterDelegate {
    ...
    weak var store: ChronoplexStore?
