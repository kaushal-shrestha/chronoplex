...
class TimerService : Service() {
    ...
    private fun startTimerAlert(timerId: Long) {
        ...
        scope.launch {
            val timer = withContext(Dispatchers.IO) {
                (applicationContext as ChronoplexApp).container.timerRepo.getById(timerId)
            }
    ...
    private fun stopTimerAlert(timerId: Long) {
        ...
        if (wasForeground) {
            ...
            scope.launch {
                val timer = withContext(Dispatchers.IO) {
                    (applicationContext as ChronoplexApp).container.timerRepo.getById(nextTimerId)
                }
                if (timer == null || timer.state != TimerState.FINISHED) {
    ...
    companion object {
        const val ACTION_START = "com.chronoplex.app.timer.service.START"
        const val ACTION_STOP = "com.chronoplex.app.timer.service.STOP"
